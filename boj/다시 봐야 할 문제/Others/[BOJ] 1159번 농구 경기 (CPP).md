...
백준온라인저지(BOJ) https://www.acmicpc.net/problem/1159
...
| :-------- | :---------- | :--- | :--- | :-------- | :-------- |
| 1 초      | 128 MB      | 4098 | 2206 | 1991      | 54.354%   |
...
int main() {
	...
	for (int i = 0; i < 26; i++) {
		if (alphabet[i] >= 5) {
			printf("%c", i + 'a'); exist = true; // alphabet[i]가 아닌 i
		}
	}
...
1. 굳이 선수들의 이름을 배열에 저장할 필요가 없다.
2. 첫 번째 반복문에서 굳이 i를 선언한 필요 없이 while문으로 n을 하나씩 빼주면 된다.